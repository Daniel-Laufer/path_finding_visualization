...
class DijkstrasAlgorithm(Algorithm):
    """
    An implementation of Dijkstra's algorithm. Note that this is nearly identical to the A* algorithm except that
    this algorithm is not accounting for any heuristic like A* did.
    ===========
        ...
        if not self.window.game_status:
            ...
            for neighbor in u.neighbors:
                ...
                if (neighbor.cost is None or alt_cost < neighbor.cost) and neighbor.color not in [STATUS_COLORS["START"], STATUS_COLORS["WALL"]]:
                    ...
                    heappush(self.q, (alt_cost, neighbor))
